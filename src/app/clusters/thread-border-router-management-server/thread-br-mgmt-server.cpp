...
static Status AddFailureStatus(CommandHandlerInterface::HandlerContext & ctx, Status status)
{
                                                   ...
                                                   const Commands::GetActiveDatasetRequest::DecodableType & req)
{
    ...
    {
        VerifyOrReturn(AddFailureStatus(ctx, Status::NotFound) == Status::Success);
    }
    else if (err != CHIP_NO_ERROR)
    {
        VerifyOrReturn(AddFailureStatus(ctx, Status::Failure) == Status::Success);
    }
                                                   ...
                                                   const Commands::SetActiveDatasetRequest::DecodableType & req)
{
    ...
    if (activeDataset.Init(req.activeDataset) != CHIP_NO_ERROR)
    {
        ...
        // of INVALID_COMMAND.
        VerifyOrReturn(AddFailureStatus(ctx, Status::InvalidCommand) == Status::Success);
    }
        ...
        currentActiveDataset.GetActiveTimestamp(currentActiveDatasetTimestamp) == CHIP_NO_ERROR)
    {
        ...
        VerifyOrReturn(AddFailureStatus(ctx, Status::InvalidInState) == Status::Success);
    }
    ...
    if (mDelegate->SetActiveDataset(activeDataset, this) != CHIP_NO_ERROR)
    {
        OnActivateDatasetComplete(Status::Failure);
    }
}
...
void ServerInstance::OnActivateDatasetComplete(Status status)
{
    ...
    if (status == Status::Success && commandHandle != nullptr)
    {
    ...
    commandHandle->AddStatus(mPath, status);
}
